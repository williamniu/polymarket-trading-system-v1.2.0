...
配置 v3 从 Betwick 最近 90 天的目标领域 material actions 出发，对每个精确 `conditionId` 反查公开成交：

- 不设最低概率价格，也不设距离到期超过 24 小时的条件；价格和到期时间只用于观察；
- 同方向必须发生在六小时内，并至少重复于三个不同的精确市场；
- 30 分钟内拆单先净额聚合，$100 作用于净方向仓位估算名义金额，防止来回成交用 gross volume 伪造观点；它不是价格门槛；
- 报告记录 signed timing，并把高度同步、持续跟在参考钱包之后的候选标成 follower warning；
- 原始证据分别记录规范化内容哈希和 gzip 文件哈希；
- 不打开 SQLite、不接入 LaunchAgent、不产生 signal、position 或 order。
