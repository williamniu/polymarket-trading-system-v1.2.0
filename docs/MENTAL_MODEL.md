...
| M4 | A signal has incremental predictive value after costs | M4.1-A2 audited 50 wallets; Betwick is one conditional observation lead, but no three-expert cohort or signal exists | Look-ahead, selection bias, linked wallets, delay decay, regime dependence, or negative out-of-sample value |
...
At `2026-08-15T21:14:51Z`, configuration v2 sealed all nine Politics/Economics/Finance leaderboard slices and public evidence for 50 of 108 recurring wallets. The same two wallets passed mechanically. Manual review keeps `betwick` only as a conditional observation lead because its public profile and interview make the approach partly explainable, while its truncated evidence, strategy drift, correlated exposure and reported 70% historical drawdown remain serious warnings. Wallet `0x06b2934b382d4429d50d7239ee375a76167f9f35` is an anonymous, short-horizon daily-index trader and is not eligible for an explainable expert cohort. Their recent samples share zero condition IDs, so they neither prove common ownership nor form same-event consensus. The user's three-independent-expert hypothesis still cannot emit a signal.
